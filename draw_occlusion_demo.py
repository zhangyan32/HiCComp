...
for start in range(2400, 7700, 50000):
    ...
    for sample_number in range (0, anchor_raw.shape[0], 10):
        ...
        original_loss =  d(output_anchor_npy[0][0], output_pos_npy[0][0]) - d(output_anchor_npy[0][0], output_neg_npy[0][0]) 
        ...
        for i in range(0, subImage_size+1 - windows_size, step):
            for j in range(0, subImage_size+1 - windows_size, step):
                ...
                new_loss = d(output_anchor_npy[n][0], output_pos_npy[n][0]) - d(output_anchor_npy[n][0], output_neg_npy[n][0])
        ...
        plt.grid(color='w')
        plt.colorbar()
        plt.savefig('sample_2400_occlude_overall_' + str(windows_size)+'_max' + str(upper), bbox_inches='tight' )
        plt.show()
        plt.close() 
        '''
